...
import { VocabularyMemoryFlashcard } from '../components/VocabularyMemoryFlashcard';
import { useMemoryReviewSession } from '../hooks/useMemoryReviewSession';

export const MemoryVocabularyReviewPage = () => {
  const navigate = useNavigate();
  const loadCards = useCallback(() => memoryApi.getCards('vocabulary', 'due'), []);
  const submitCardAnswer = useCallback((memoryItemId: string, quality: number) => memoryApi.submitAnswer('vocabulary', memoryItemId, quality), []);
  ...
  const {
    ...
    loadCards,
    submitCardAnswer,
    loadErrorMessage: 'Không tải được thẻ từ vựng cần ôn.',
    submitErrorMessage: 'Không lưu được kết quả ôn tập từ vựng.',
  });
  ...
  if (isLoading) {
    return (
      <div className="flex h-64 flex-col items-center justify-center text-text-secondary">
        ...
        <p className="font-bold">Đang tải thẻ từ vựng...</p>
      </div>
    );
  }
  ...
  if (queue.length === 0) {
    return (
      <div className="mx-auto max-w-3xl space-y-5 rounded-2xl bg-white p-8 text-center shadow-card">
        ...
        <h1 className="text-3xl font-black text-text-primary">Không có từ vựng cần ôn</h1>
        <p className="font-bold text-text-secondary">Hãy mở một bài từ vựng và bấm "Ghi nhớ" ở từ bạn muốn học.</p>
    ...
    return (
      <div className="mx-auto max-w-4xl space-y-6 rounded-2xl bg-white p-8 shadow-card">
        <div className="text-center">
          ...
          <p className="mt-2 font-bold text-text-secondary">Bạn đã xử lý {answeredCount} lượt card từ vựng trong Ghi nhớ.</p>
        </div>
