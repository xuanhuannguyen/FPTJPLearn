...
import { KanjiMemoryFlashcard } from '../components/KanjiMemoryFlashcard';
...
export const MemoryKanjiReviewPage = () => {
  const navigate = useNavigate();
  const loadCards = useCallback(() => memoryApi.getCards('kanji', 'due'), []);
  const submitCardAnswer = useCallback((memoryItemId: string, quality: number) => memoryApi.submitAnswer('kanji', memoryItemId, quality), []);
  ...
  const {
    ...
    loadCards,
    submitCardAnswer,
    loadErrorMessage: 'Không tải được thẻ Kanji cần ôn.',
  ...
  if (queue.length === 0) {
    return (
      <div className="mx-auto max-w-3xl space-y-5 rounded-2xl bg-white p-8 text-center shadow-card">
        ...
        <h1 className="text-3xl font-black text-text-primary">Không có Kanji cần ôn</h1>
        <p className="font-bold text-text-secondary">Hãy mở một chữ Kanji và bấm "Thêm vào ghi nhớ".</p>
    ...
    return (
      <div className="mx-auto max-w-4xl space-y-6 rounded-2xl bg-white p-8 shadow-card">
        <div className="text-center">
          ...
          <p className="mt-2 font-bold text-text-secondary">Bạn đã xử lý {answeredCount} lượt card Kanji trong Ghi nhớ.</p>
        </div>
