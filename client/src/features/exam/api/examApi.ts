...
export const examApi = {
  ...
  getTopics: async (courseCode?: string, level?: string): Promise<ExamTopic[]> => {
    const response = await apiClient.get<{ topics: ExamTopic[] }>('/exam/topics', {
      params: { courseCode, level },
    });
  ...
  answerQuestion: async (questionId: string, selectedOptionId: string): Promise<ExamAnswerResult> => {
    const response = await apiClient.post<ExamAnswerResult>(`/exam/questions/${questionId}/answer`, {
      selectedOptionId,
    });
    return response.data;
  },
  ...
  saveAttemptAnswer: async (attemptId: string, questionId: string, selectedOptionId: string): Promise<ExamAttemptAnswer> => {
    const response = await apiClient.post<ExamAttemptAnswer>(`/exam/attempts/${attemptId}/answers`, {
      questionId,
      selectedOptionId,
    });
    return response.data;
